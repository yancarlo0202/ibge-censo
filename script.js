document.addEventListener('DOMContentLoaded', () => {
            ...
            function handleFileLoad(event) {
                ...
                Papa.parse(file, {
                    header: true, skipEmptyLines: true, delimiter: ";", encoding: "latin1", dynamicTyping: false,
                    ...
                    complete: (results) => {
                        ...
                        const missingHeaders = requiredHeaders.filter(h => !actualHeaders.includes(h));
                        if (results.errors.length > 0 || missingHeaders.length > 0) {
                            showToast(`Erro no ficheiro. Verifique as colunas: ${missingHeaders.join(', ')}`, 'error');
                            fileStatus.textContent = "Falha no carregamento."; return;
                        }
                        dadosCompletos = results.data.map((row, index) => ({
                            ...row, row_id: index, geocodigo: String(row.geocodigo || '').trim(),
                            distancia_sede: parseFloat(String(row.distancia_sede || '0').replace(',', '.')) || 0,
                            trajeto_diario: parseFloat(String(row.trajeto_diario || '0').replace(',', '.')) || 0,
                            estabelecimentos: parseInt(row.estabelecimentos, 10) || 0
                        }));
                        fileStatus.textContent = `Ficheiro "${file.name}" carregado com sucesso!`;
                        showToast('Ficheiro CSV carregado com sucesso!', 'success');
                        mainContent.classList.remove('disabled-section'); popularAgencias();
                    },
                    error: (err) => { showToast(`Não foi possível ler o ficheiro: ${err.message}`, 'error'); fileStatus.textContent = "Falha no carregamento."; }
                });
            }
            ...
            function atualizarMunicipios() {
                ...
                if (!agenciaSelecionada) { municipioContainer.classList.add('disabled-section'); return; }
            ...
            function atualizarGeocodigos() {
                const municipioSelecionado = municipioSelect.value;
                geocodigosContainer.innerHTML = '';
                if (!municipioSelecionado) {
                    geocodigosContainer.innerHTML = '<p class="text-gray-400 text-center mt-4">Selecione um município para ver os setores.</p>';
                    setoresContainerWrapper.classList.add('disabled-section'); return;
                }
                dadosCompletos.filter(item => item.municipio === municipioSelecionado).forEach(item => {
                    ...
                    checkbox.type = 'checkbox'; checkbox.value = item.row_id; checkbox.id = `geo-${item.row_id}`;
            ...
            function adicionarViagem() {
                ...
                const novaViagem = { id: Date.now(), nomeServidor, siapeServidor, cargoServidor, agencia, municipio, setores };
            ...
            function resetarFormularioCadastro() {
                ...
                document.getElementById('siape-servidor').value = '';
                document.getElementById('cargo-servidor').value = '';
                agenciaSelect.selectedIndex = 0;
                atualizarMunicipios();
            }
            ...
            function renderChart(data, mode) {
                ...
                costsChart = new Chart(ctx, {
                    type: 'bar',
                    data: { labels, datasets: [{ label: 'Valor em R$', data: chartData, backgroundColor: ['rgba(54, 162, 235, 0.6)', 'rgba(75, 192, 192, 0.6)', 'rgba(255, 99, 132, 0.6)'], borderColor: ['rgba(54, 162, 235, 1)', 'rgba(75, 192, 192, 1)', 'rgba(255, 99, 132, 1)'], borderWidth: 1 }] },
                    options: { responsive: true, scales: { y: { beginAtZero: true, ticks: { callback: value => 'R$ ' + value.toFixed(2) } } }, plugins: { legend: { display: false }, title: { display: true, text: chartTitle, font: { size: 16 } } } }
                });
            }

            function updateModalidadeOptionsCalc(event) {
                const modalidade = event.target.value;
                veiculoOptionsContainerCalc.innerHTML = ''; custosContainerCalc.innerHTML = '';
            ...
            function switchTab(tabName) {
                if (tabName === 'cadastro') {
                    cadastroView.classList.remove('hidden'); calculoView.classList.add('hidden');
                    tabCadastro.classList.replace('border-transparent', 'border-indigo-500'); tabCadastro.classList.replace('text-gray-500', 'text-indigo-600');
                    tabCalculadora.classList.replace('border-indigo-500', 'border-transparent'); tabCalculadora.classList.replace('text-indigo-600', 'text-gray-500');
                } else {
                    cadastroView.classList.add('hidden'); calculoView.classList.remove('hidden');
                    tabCalculadora.classList.replace('border-transparent', 'border-indigo-500'); tabCalculadora.classList.replace('text-gray-500', 'text-indigo-600');
                    tabCadastro.classList.replace('border-indigo-500', 'border-transparent'); tabCadastro.classList.replace('text-indigo-600', 'text-gray-500');
                }
            }
            ...
            modalidadeContainerCalc.addEventListener('change', updateModalidadeOptionsCalc);
            finalizarCalculoBtn.addEventListener('click', () => {
                ...
                if (modalidade === 'misto') {
                    ...
                    distanciaTerrestreInput.value = ''; distanciaFluvialInput.value = '';
                    diasDiariaTerrestreMistoInput.value = ''; diasDiariaFluvialMistoInput.value = '';
                    mistoModalBackdrop.classList.remove('hidden'); mistoModalBackdrop.classList.add('flex');
                } else {
                    finalizarCalculo();
                }
            });
            ...
            confirmarMistoBtn.addEventListener('click', () => {
                ...
                if (Math.abs((distT + distF) - totalDist) > 0.01) { showToast('A soma das distâncias deve ser igual à distância total.', 'error'); return; }
                if (diasT > totalDias || diasF > totalDias) { showToast(`As diárias não podem exceder o total de dias da viagem (${totalDias}).`, 'error'); return; }
